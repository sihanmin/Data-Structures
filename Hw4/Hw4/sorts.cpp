...
int main()
{
    ...
    // destruction time.
    {
        ...
        {
            studentsptr.push_back(&auxStudents[i]);
        }
        
        // TODO:  Sort the vector of pointers using the STL sort algorithm
        //        with compareStudentPtr as the ordering relationship.
        for(int i = 2; i <= studentsptr.size(); i++)
        {
            Student* sortMe = studentsptr[ i - 1 ];
            
            int j = i - 2;
            while (j >= 0 && !compareStudentPtr(studentsptr[j], sortMe))
            {
                studentsptr[j+1] = studentsptr[j];
                j--;
            }
            
            studentsptr[j+1] = sortMe;
        }
